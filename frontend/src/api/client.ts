...
export const api = {
  ...
  getDonor: (id: string, donorIdHeader?: string) =>
    request<{ donor: any; eligibility: any }>(`/donors/${id}`, {
      // NOTE: getStoredAuthHeaders now handles the auth token which carries the custom:role
      // so we don't strictly need to pass x-user-id here if the backend is using the token,
      // but keeping custom headers for compatibility if needed.
      method: "GET",
    }),
